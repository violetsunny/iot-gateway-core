...
import top.iot.gateway.core.metadata.UnitSupported;
import top.iot.gateway.core.metadata.ValidateResult;
import top.iot.gateway.core.utils.NumberUtils;
...
public abstract class NumberType<N extends Number> extends AbstractType<NumberType<N>> implements UnitSupported, DataType, Converter<N> {
    ...
    static boolean ORIGINAL = Boolean.parseBoolean(System.getProperty("iot.gateway.type.number.convert.original", "true"));
    ...
    protected RoundingMode defaultRound() {
        return RoundingMode.valueOf(System.getProperty("iot.gateway.type." + getType() + ".round", "HALF_UP"));
    }
